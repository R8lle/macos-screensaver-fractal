...
/// Renders the built FractalSaver.saver and writes System Settings picker thumbnails.
/// Usage: capture_thumbnail <FractalSaver.saver> <output-dir>
...
guard CommandLine.arguments.count >= 3 else {
    fputs("Usage: capture_thumbnail <FractalSaver.saver> <output-dir>\n", stderr)
    exit(1)
}
...
// Drive a few frames explicitly (ScreenSaver timer may not fire in this host).
if let mtk = findMTKView(in: view) {
    for _ in 0..<90 {
        mtk.draw()
...
if let full = resizedPNG(from: captured, width: 1280, height: 800) {
    write(full, to: "\(outDir)/thumbnail-full.png")
}
